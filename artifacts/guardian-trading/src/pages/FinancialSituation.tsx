...
function SelectList({
  ...
  return (
    <div className="flex-1">
      {label && (
        <p className="font-bold mb-2 uppercase" style={{ fontSize: "12px", color: "#444", letterSpacing: "0.04em" }}>{label}</p>
      )}
      <div style={{ border: `1px solid ${error ? "#e53e3e" : "#dde3e9"}`, borderRadius: "2px", overflow: "hidden" }}>
        {options.map((opt, i) => {
          ...
          const isEven = i % 2 === 0;
          return (
            <div key={opt} onClick={() => onSelect(opt)} className="cursor-pointer" style={{ padding: "7px 12px", fontSize: "13px", color: "#444", background: isSel ? "#cfe1f5" : isEven ? "#ffffff" : "#edf1f5", userSelect: "none", display: "flex", alignItems: "center", justifyContent: "space-between", transition: "background 0.15s ease" }}>
              <span>{opt}</span>
              {isSel && (
...
export default function FinancialSituation() {
  ...
  const [errors, setErrors] = useState<{ annualExpense?: string; specialExpense?: string; liquidity?: string; timeHorizon?: string }>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrors({});
    await saveSignupStep("financialSituation", { annualExpense, specialExpense, liquidityNeeds: liquidity, investmentTimeHorizon: timeHorizon });
        ...
        <div className="px-8 py-6">
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => navigate("/risk-tolerance")} className="font-medium hover:bg-gray-50 transition-colors" style={{ padding: "9px 28px", border: "1px solid #ccd3da", borderRadius: "3px", background: "white", fontSize: "13px", color: "#555", cursor: "pointer" }}>Previous</button>
