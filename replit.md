...
- **Deployment**: Replit VM deployment builds both Vite frontends and the Express API server, then runs the API server as the single production entry point. The Express server serves the built customer app at `/`, the admin portal at `/admin-kyc`, and API routes under `/api`.
