...
- **Rate limiting**: AI chat (20 req/min), file uploads (30 req/15min), auth endpoints (30 req/15min), market data (30 req/min), user data / notifications (60 req/min), change-password (30 req/15min via sensitiveEndpointLimit), global (100 req/min)
- **Input validation**: All endpoints accepting email parameters validate email format with regex; profile update validates name format, phone format, and location format with regex before saving
