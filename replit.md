...
- **Real-Time Admin Sync**: WebSocket channel `guardian-admin` delivers `NEW_USER_REGISTRATION` and `APPLICATION_COMPLETE` events instantly. `useAdminRealtime` hook connects with 65s heartbeat timeout, auto-reconnect, and localStorage persistence. Backend broadcasts from `server/routes/auth.ts` (on register) and `server/routes/signup.ts` (on all 12 steps complete).
- **Registration Log**: `registration_log` PostgreSQL table (TEXT columns: email, display_name, referrer, product, registration_type, ip_address) persists every new signup. Admin view at `/admin-kyc → Registrations` shows expandable rows with full decrypted KYC profile, document image thumbnails (click to enlarge), and comprehensive CSV export including all 12 onboarding steps.
- **Admin Toast System**: `AdminToast.tsx` — dark green/black frosted-glass card matching the design reference, bottom-right positioned, supports `registration`, `kyc_complete`, `success`, `info`, `error` types with animated icon, slide-in animation, 8s auto-dismiss.
