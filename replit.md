...
- **Deployment**: External production deployment is configured for Render as a single Node web service. The build creates both Vite frontends and the Express API server, then the Express server serves the customer app at `/`, the admin portal at `/admin-kyc`, and API routes under `/api`. Static hosts like Netlify/Vercel require `VITE_API_URL` to point to the deployed API domain, such as `https://api.guardiiantrading.com`.
...
- **CORS**: Production-locked — only allows explicit origins: `guardiiantrading.com`, `www.guardiiantrading.com`, and `guardian-clone-production.up.railway.app`. No wildcard matching. Dev mode adds exact REPLIT_DEV_DOMAIN + localhost ports only. CORS rejections logged with origin info. Admin-kyc uses relative `/api/` URLs in dev mode (same-origin through Replit proxy), Railway production URL only in production builds. Bot detection middleware is disabled in non-production mode.
